...
2. ### How can you avoid callback hells?
   ...
   1. Modularization:
   ```js
   function handleWallet(result) {
    console.log(result);
    }

    function handlePayment(payment) {
    updateWallet(payment, handleWallet);
    }
   ...
   2. Controle Flow Libraries(like async)
   Before Promises were popular, libraries helped manage async flow.
   ```js
   async.series([
        function(cb) { getUser(id, cb); },
        function(cb) { getOrders(cb); },
        function(cb) { getPayment(cb); }
   ...
   function* flow() {
        ...
        const payment = yield getPayment(orders[0].id);
        return payment;
    }
   ```
     Yield pause execution
     Promise resolves --> resumes
   
   4. Async/Await(Modern and Best)
   ...
   ```js
    async function processUser(id) {
        ...
        const result = await updateWallet(payment);
        console.log(result);
        }
   ```
...
3. ### When are background or worker processes useful?
   ...
   Background worker processes are used for tasks that are time-consuming or CPU-intensive and don’t need to complete during the request-response cycle. They offload heavy work like image processing, email sending, and data pipelines to separate processes, keeping the main Node.js server responsive and scalable. This is typically implemented using job queues like RabbitMQ, Kafka, or Redis-based systems.
   ...
   - Worker = task executor
    ...
    ``` User Request
        ...
        API Server (Node.js)
            ↓
        Job Queue  ← stores tasks
            ↓
        Worker Process (Background)
            ↓
        DB / Storage / Email Service
    ```
...
4. ### Why is NodeJS Single threaded?
   ...
   A thread = a worker that executes code.
   Traditional servers (Java, C#, etc.)
    - One request → One thread
    - 1000 users → 1000 threads
   Node.js
    - One main thread handles all requests
    - No thread per user
    So Node doesn’t create a new worker for every request.

    Blocking (Thread Style)
    ```js
        const data = db.query("SELECT * FROM users"); // wait here, server is stuck and waiting
        console.log(data);
    ```
    ...
    ```js
        ...
        console.log("Next request handled immediately");
    ```
    while DB works, Node handles other users

    Node.js uses a single-threaded event loop to handle many concurrent requests efficiently. Instead of creating a thread per request, it uses non-blocking asynchronous I/O. When an operation like a database call happens, Node delegates it to the system and continues serving other requests. This avoids thread overhead, reduces memory usage, and improves scalability for I/O-heavy applications.
...
45. ###	Concurrency vs Parallelism
    Concurrency = Dealing with many tasks at once ( Single chef, many dishes)
    Parallelism = Doing many tasks at the exact same time ( Multiple chefs)

    🟡 Concurrency
    - Tasks progress together
    - May run on single CPU
    - Uses task switching
    - Good for waiting operations (I/O)
    - CPU switches between tasks.
    - Use cases: APIs, Web servers, chat apps, DB calls, Network requests

    🔵 Parallelism
    - Tasks execute simultaneously
    - Needs multiple CPUs/cores
    - True simultaneous execution
    - Good for heavy computation
    - All(cores or cpus) run at same time.
    - Use cases: Video encoding, Machine learning, Image Processing, Data crunching

    Node.js uses an event-driven, non-blocking architecture that enables high concurrency on a single thread. For CPU-intensive workloads, we use worker threads or clustering to leverage parallelism across multiple cores.
